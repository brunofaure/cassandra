...
//        Clock timestamp = new Clock(System.nanoTime());
        long timestamp = System.currentTimeMillis();
        
        Column nameCol = new Column(hotelName.getBytes(UTF8), new byte[0], timestamp);
        ...
        Column col = new Column(hotelName.getBytes(UTF8), new byte[0], timestamp);

        client.insert(rowKey.getBytes(), columnParent, col, CL);
    ...
    private void insertPOISpringTraining() throws Exception {
        ...
        Column descCol = new Column("desc".getBytes(UTF8),
                "Fun for baseball fans.".getBytes("UTF-8"), timestamp);
        
        Column phoneCol = new Column("phone".getBytes(UTF8),
                "623-333-3333".getBytes(UTF8), timestamp);
    ...
    private void insertPOICentralPark() throws Exception {
        ...
        Column descCol = new Column("desc".getBytes(UTF8),
                "Walk around in the park. It's pretty.".getBytes("UTF-8"), timestamp);
    ...
    private void insertPOIEmpireState() throws Exception {
        ...
        Column descCol = new Column("desc".getBytes(UTF8),
                "Great view from 102nd floor.".getBytes("UTF-8"), timestamp);
        Column phoneCol = new Column("phone".getBytes(UTF8),
                "212-777-7777".getBytes(UTF8), timestamp);
...
//batch_mutate(java.util.Map<java.nio.ByteBuffer,java.util.Map<java.lang.String,java.util.List<Mutation>>> mutation_map, ConsistencyLevel consistency_level) 
...
//        Clock timestamp = new Clock(System.nanoTime());
           ...
           Column nameCol = new Column(toByteBuffer("name"));
           nameCol.setValue(toByteBuffer(W_NAME));
           nameCol.setTimestamp(timestamp);
        
        Column phoneCol = new Column("phone".getBytes(UTF8),
                "415-222-2222".getBytes(UTF8), timestamp);
        Column addressCol = new Column("address".getBytes(UTF8),
                "181 3rd Street".getBytes(UTF8), timestamp);
        Column cityCol = new Column("city".getBytes(UTF8),
                "San Francisco".getBytes(UTF8), timestamp);
        Column stateCol = new Column("state".getBytes(UTF8),
                "CA".getBytes("UTF-8"), timestamp);
        Column zipCol = new Column("zip".getBytes(UTF8),
                "94103".getBytes(UTF8), timestamp);
        ...
        //outer map key is a row key
        //inner map key is the column family name
        mutationMap.put(toByteBuffer(rowKey), muts);
        return mutationMap;
    }
...
//        Clock timestamp = new Clock(System.nanoTime());
        ...
        Column nameCol = new Column("name".getBytes(UTF8),
                WALDORF_NAME.getBytes("UTF-8"), timestamp);
        Column phoneCol = new Column("phone".getBytes(UTF8),
                "212-555-5555".getBytes(UTF8), timestamp);
        Column addressCol = new Column("address".getBytes(UTF8),
                "301 Park Ave".getBytes(UTF8), timestamp);
        Column cityCol = new Column("city".getBytes(UTF8),
                "New York".getBytes(UTF8), timestamp);
        Column stateCol = new Column("state".getBytes(UTF8),
                "NY".getBytes("UTF-8"), timestamp);
        Column zipCol = new Column("zip".getBytes(UTF8),
                "10019".getBytes(UTF8), timestamp);
...
//        Clock timestamp = new Clock(System.nanoTime());
        ...
        Column nameCol = new Column("name".getBytes(UTF8),
                CLARION_NAME.getBytes("UTF-8"), timestamp);
        Column phoneCol = new Column("phone".getBytes(UTF8),
                "480-333-3333".getBytes(UTF8), timestamp);
        Column addressCol = new Column("address".getBytes(UTF8),
                "3000 N. Scottsdale Rd".getBytes(UTF8), timestamp);
        Column cityCol = new Column("city".getBytes(UTF8),
                "Scottsdale".getBytes(UTF8), timestamp);
        Column stateCol = new Column("state".getBytes(UTF8),
                "AZ".getBytes("UTF-8"), timestamp);
        Column zipCol = new Column("zip".getBytes(UTF8),
                "85255".getBytes(UTF8), timestamp);
...
//        Clock timestamp = new Clock(System.nanoTime());
        long timestamp = System.currentTimeMillis();
           ...
           Column cambriaNameCol = new Column(toByteBuffer("name"));
           cambriaNameCol.setValue(toByteBuffer("Cambria Suites Hayden"));
           cambriaNameCol.setTimestamp(timestamp);
           
           Column cambriaPhoneCol = new Column(toByteBuffer("phone"));
           cambriaPhoneCol.setValue(toByteBuffer("480-444-4444"));
           cambriaPhoneCol.setTimestamp(timestamp);
           
           Column cambriaAddressCol = new Column(toByteBuffer("address"));
           cambriaAddressCol.setValue(toByteBuffer("400 N. Hayden"));
           cambriaAddressCol.setTimestamp(timestamp);
           
           Column cambriaCityCol = new Column(toByteBuffer("city"));
           cambriaCityCol.setValue(toByteBuffer("Scottsdale"));
           cambriaCityCol.setTimestamp(timestamp);
           
           Column cambriaStateCol = new Column(toByteBuffer("state"));
           cambriaStateCol.setValue(toByteBuffer("AZ"));
           cambriaStateCol.setTimestamp(timestamp);
           
           Column cambriaZipCol = new Column(toByteBuffer("zip"));
           cambriaZipCol.setValue(toByteBuffer("85255"));
           cambriaZipCol.setTimestamp(timestamp);
        ...
        //outer map key is a row key
        //inner map key is the column family name
        cambriaMutationMap.put(toByteBuffer(cambriaKey), cambriaMuts);
